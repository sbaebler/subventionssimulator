...
<div class="mb-6">
  <a href="/events.php" class="text-sm link-muted">&larr; Zurück zu den Events</a>
</div>

<?php if (!$event): ?>
  <p class="empty-state">Event nicht gefunden.</p>
<?php else: ?>
  ...
  <div class="mb-6">
    <div class="flex items-center gap-2 mb-1">
      <span class="badge <?= Event::STATUS_BADGE[$event['status']] ?? 'badge--neutral' ?>">
        <?= htmlspecialchars(Event::STATUS[$event['status']] ?? $event['status']) ?>
      </span>
      <?php if ($event['klasse_bezeichnung']): ?>
      <span class="text-xs text-muted"><?= htmlspecialchars($event['klasse_bezeichnung']) ?></span>
      <?php endif; ?>
    </div>
  ...
  <form method="post" action="/events_zuordnen.php?id=<?= $event['id'] ?>">
    ...
    <h2 class="text-sm font-medium mb-3">Welche Subventionen kommen für dieses Event zum Tragen?</h2>

    <?php if (empty($subventionen)): ?>
      <p class="text-muted text-sm">Noch keine Subventionen erfasst.
        <a href="/erfassen.php" class="link">Jetzt erfassen</a>
      </p>
    <?php else: ?>
      <div class="grid gap-2 mb-6">
        <?php foreach ($subventionen as $s): ?>
        <label class="card flex items-start gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50">
          <input type="checkbox" name="subventionen[]" value="<?= $s['id'] ?>"
                 class="checkbox mt-1"
                 <?= in_array((int)$s['id'], $zugeordnet, true) ? 'checked' : '' ?>>
          <span>
            <span class="font-medium"><?= htmlspecialchars($s['bezeichnung']) ?></span>
            <span class="block text-sm text-muted"><?= htmlspecialchars($s['foerderstelle']) ?></span>
          </span>
        </label>
        <?php endforeach; ?>
      </div>
